...
def hinge_loss_single(feature_vector, label, theta, theta_0):
    ...
    hinge_loss = max(0.0, 1 - score*label)
    return hinge_loss
...
def hinge_loss_full(feature_matrix, labels, theta, theta_0):
    ...
    """
    hinge_loss_total = 0.0
    n = feature_matrix.shape[0]
    for i in range(n):
        hinge_loss_total += hinge_loss_single(feature_matrix[i], labels[i], theta, theta_0)
    return hinge_loss_total/n
...
def average_perceptron(feature_matrix, labels, T):
    ...
    for t in range(T):
        for i in get_order(nsamples):
            ...
            theta_total += theta
            theta_0_total += theta_0
    return theta_total/(nsamples*T),theta_0_total/(nsamples*T)
...
def classify(feature_matrix, theta, theta_0):
    ...
    """
    nsamples = feature_matrix.shape[0]
    result = np.full(nsamples, -1, dtype=float)
    epsilon = .000000001
    for i in range(nsamples):
        score = np.dot(theta, feature_matrix[i]) + theta_0
        if score > epsilon:
            result[i] = 1
    return result
...
def classifier_accuracy(
    ...
    """
    # Fit on training data, get classifier weights (could be e.g. theta, theta_0 or similar)
    params = classifier(train_feature_matrix, train_labels, **kwargs)
    # Classify training data
    train_preds = classify(train_feature_matrix, *params)
    train_acc = accuracy(train_preds, train_labels)
    # Classify validation data
    val_preds = classify(val_feature_matrix, *params)
...
def bag_of_words(texts, remove_stopword=True):
    ...
    # # Your code here
    # raise NotImplementedError
    # stopword = {'beach', 'better', 'loves', 'nothing', 'walk'}

    # stopwords_location = 'stopwords.txt'
    base_dir = os.path.dirname(os.path.abspath(__file__))
    stopwords_location = os.path.join(base_dir, 'stopwords.txt')
    with open(stopwords_location, 'r') as f:
        stopword = {line.strip() for line in f if line.strip()}
...
def extract_bow_feature_vectors(reviews, indices_by_word, binarize=False):
    ...
    binary_matrix = np.zeros([len(reviews), len(indices_by_word)], dtype=np.float64)
    for i, text in enumerate(reviews):
        word_list = extract_words(text)
        for word in word_list:
            if word not in indices_by_word: continue
            feature_matrix[i, indices_by_word[word]] += 1
            binary_matrix[i, indices_by_word[word]] = 1
    if binarize:
