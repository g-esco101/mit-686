...
def compute_probabilities(X, theta, temp_parameter):
    ...
    probabilities = scores_exp / np.sum(scores_exp, axis=0, keepdims=True)
...
def compute_test_error_mod3(X, Y, theta, temp_parameter):
    ...
    """
    # Predict digits (0–9) with current model
    y_hat_digits = get_classification(X, theta, temp_parameter)   # shape (n,)

    # Reduce labels modulo 3
    y_hat_mod3 = y_hat_digits % 3
    y_true_mod3 = Y % 3

    # Error rate = 1 - accuracy
    test_error = 1.0 - np.mean(y_hat_mod3 == y_true_mod3)
    return test_error
