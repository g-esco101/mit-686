...
def project_onto_PC(X, pcs, n_components, feature_means):
    ...
    X_centered = X - feature_means  # broadcasts over rows

    # Take first n_components principal axes (columns)
    V = pcs[:, :n_components]       # (d, k)
...
def plot_PC(X, pcs, labels, feature_means):
    ...
    this function mit-686-projects each sample in X onto the first two principal components
