...
def plot(X: np.ndarray, mixture: GaussianMixture, post: np.ndarray,
    ...
    for i, point in enumerate(X):
        ...
        for j in range(K):
            ...
            # Some versions of matplotlib expect only (xy, width, height, angle)
            # as positional args; theta1 and theta2 should be passed as keywords.
            arc = Arc(point,
                      r,
                      r,
                      angle=0,
...
def bic(X: np.ndarray, mixture: GaussianMixture,
    ...
    n, d = X.shape
    K = len(mixture.mu)
    # p = (mu params) + (variance params) + (p params)
    p = (K*d) + (K) + (K-1)
